...
def recursiveGetStatus(url, method, body, delay):
    while True:
        ...
        if data:
            ...
            cursor = conn.cursor()
            try:
                cursor.execute("INSERT INTO statuses (battery, chargeFlag, emergencyButton, timeOfResponse) VALUES (%s, %s, %s)", (data["battery"], data["chargeFlag"], data["emergencyButton"], datetime.now()))
                conn.commit()
...
def executeRequest(requestPath, requestBody, needStatusCheck):
    try:
...
def checkForRequests():
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM requests WHERE status = NULL")
        requests = cursor.fetchall()
        for request in requests:
            executeRequest(request[0], request[1],request[4])        
    except psycopg2.Error as e:
...
if conn:
    t1 = threading.Thread(target=recursiveGetStatus, args=("http://" + robotIP + "/reeman/base_encode", "GET", None, 20)) # thread to constantly get statuses
